...
public class ParticipantesDAOTest {
    ...
    public void testObtenerParticipantesConFiltro() throws SeguridadException {
        ...
        List<ParticipanteDTO> resultadosNumControl = participantesDAO.obtenerParticipantesConFiltro("00000111111");
        ...
        for (Participante p : participantesCreadosEnEsteTest) {
            if (p != null && p.getId() != null) {
                try {
                    ...
                    System.err.println("Error al limpiar participante con ID " + p.getId() + ": " + e.getMessage());
                }
            }
        }
    }
    ...
    public void testObtenerParticipantesConFiltroYTipo() throws SeguridadException {
        ...
        assertTrue(resTipoEstudiante.stream().allMatch(p -> p.getTipoParticipante() == TipoParticipante.ESTUDIANTE));
        ...
        assertEquals(3, resSoloTexto.size(), "Deberían haber 3 participantes con 'Pérez' en su nombre/apellido");
        assertTrue(resSoloTexto.stream()
                .anyMatch(p -> p.getNombre().equals("Juan Carlos") && p.getApellidoPaterno().equals("Pérez"))); // Juan
                                                                                                                // Carlos
                                                                                                                // Pérez
        assertTrue(resSoloTexto.stream()
                .anyMatch(p -> p.getNombre().equals("Carlos Alberto") && p.getApellidoPaterno().equals("Pérez"))); // Carlos
                                                                                                                   // Alberto
                                                                                                                   // Pérez
        assertTrue(resSoloTexto.stream()
                .anyMatch(p -> p.getNombre().equals("Pedro") && p.getApellidoMaterno().equals("Pérez"))); // Pedro
                                                                                                          // Sánchez
                                                                                                          // Pérez
        ...
        List<ParticipanteDTO> resTextoNoExiste = participantesDAO.obtenerParticipantesConFiltroYTipo("XYZ123ABC", null);
        ...
        List<ParticipanteDTO> resTextoNoExisteConTipo = participantesDAO.obtenerParticipantesConFiltroYTipo("XYZ123ABC",
                TipoParticipante.ESTUDIANTE);
        ...
        List<ParticipanteDTO> resCorreoYTipo = participantesDAO
                .obtenerParticipantesConFiltroYTipo("juan.estudiante@example.com", TipoParticipante.ESTUDIANTE);
        ...
        List<ParticipanteDTO> resNumControlYTipo = participantesDAO.obtenerParticipantesConFiltroYTipo("0000012345",
                TipoParticipante.ESTUDIANTE);
